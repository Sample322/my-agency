...
class Renderer {
  ...
  private color: [number, number, number] = [0.486, 0.227, 0.929]; // #7C3AED
  ...
  render(now = 0) {
    ...
    gl.uniform2f(
      (program as Record<string, WebGLUniformLocation>).resolution,
      canvas.width,
      canvas.height
    );
    gl.uniform1f(
      (program as Record<string, WebGLUniformLocation>).time,
      now * 1e-3
    );
    gl.uniform3fv(
      (program as Record<string, WebGLUniformLocation>).u_color,
      this.color
    );
...
/* Wrapper that renders children over a lazy-mounted smoke canvas */
...
export const SmokeSection: React.FC<SmokeSectionProps> = ({
  children,
  className = "",
  smokeColor = "#7C3AED",
}) => {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        smoke.style.transform = `translateY(${-offset}px) scale(1.3)`;
        ticking = false;
      });
    };
  ...
  return (
    <div ref={ref} className={`relative overflow-hidden ${className}`}>
      {visible && (
        ...
        >
          <SmokeBackground smokeColor={smokeColor} className="w-full h-full block opacity-35" />
        </div>
      )}
